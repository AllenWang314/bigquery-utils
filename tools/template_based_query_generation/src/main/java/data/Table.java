...
public class Table {
  ...
  public List<Pair<String, DataType>> getSchema() {
    return this.schema;
  }
  ...
  public String getRandomColumn(DataType type) {
    List<Pair<String, DataType>> columns = new ArrayList<Pair<String, DataType>>();
  ...
  public List<List<?>> generateData(int numRows) {
    ...
    for (int i = 0; i < this.schema.size(); i++) {
      List<?> column = this.generateColumn(numRows, this.schema.get(i).second);
      data.add(column);
    }
    return data;
  }
